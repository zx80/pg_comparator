/* $Id: checksum.c 199 2004-08-26 07:50:44Z coelho $
 *
...
/* randomly-generated byte permutation */
static uint32 perm[] = { 
...
{
	uint32 
		h1 = h ^ perm[(h+7*c+11*n) & 0xff],
		h2 = (3*h) ^ (5*c) ^ (13*n) ^ perm[c & 0xff],
		h3 = c ^ perm[(((h*(c+n)*(c-n))%104851)+h+c+n) & 0xff],
		h4 = n ^ perm[((h>>(n%17))+c) & 0xff];
